...
This is an adapter for NextAuth that allows it to interface with Redis. This is primarily a package for users looking to use NextAuth.js' reliability and security at scale. Because of the inherent complexity of managing data at scale, operations in this package have a higher chance of failing. We make **zero** assumptions about how you want to handle failed operations. This package is designed to be as unopinionated as possible, and to allow you to build your own solution on top of it. Our provided backend (while highly configurable) is more opinionated, but we can assure you that it has the correct opinions. Please note that this adapter is incapable of being used without a backend, as the backend is responsible for setting the data in the cache, which is how this adapter retrieves data.

### Use Cases
- You are planning a project that uses Redis as a database or cache. [Or not far enough along to not make breaking changes over small things. I wouldn't judge you.]
...
- Persistent database management offloaded to the backend behind a Redis Queue.
...
- Ensures that all operations are idempotent and atomic, allowing for a more reliable system at scale.
- Out-of-the-box support for Redis Sentinel and Redis Cluster ([See Below](#scaling)).
- Type definitions for TypeScript.

# Persistence
This package uses a Redis Queue system to communicate with a backend, which handles communication with a persistent database. For example, if a user is created, we push a request to store that data to the 'USER_DATA' queue. This list is then consumed by a backend process that handles the actual storage of the data. 

By default, no data is stored persistently, and the Redis Adapter is used as a cache. This is useful for development, or for applications that do not require persistent storage. Individual tables may be made persistent by setting the corresponding `persistent` flag to `true` when initializing this adapter. How-to's, sensible defaults, and examples are established in the [Configuration](#configuration) section.
...
- `ENQUEUE CREATE_USER { ...user, idempotency_key }`: This is enqueued when a user is to be created.
- `ENQUEUE UPDATE_USER { ...user, idempotency_key }`: This is enqueued when a user is to be updated.
- `ENQUEUE DELETE_USER { user_id, idempotency_key }`: This is enqueued when a user is to be deleted.
- `ENQUEUE CREATE_SESSION { ...session, idempotency_key }`: This is enqueued when a session is to be created.
- `ENQUEUE UPDATE_SESSION { ...session, idempotency_key }`: This is enqueued when a session is to be updated.
- `ENQUEUE DELETE_SESSION { session_id, idempotency_key }`: This is enqueued when a session is to be deleted.
- `ENQUEUE CREATE_VERIFICATION_REQUEST { ...verification_request, idempotency_key }`: This is enqueued when a verification request is to be created.
- `ENQUEUE USE_VERIFICATION_REQUEST { ...verification_request, idempotency_key }`: This is enqueued when a verification request is to be used.
- `ENQUEUE LINK_ACCOUNT { ...account, idempotency_key }`: This is enqueued when an account is to be linked.
- `ENQUEUE UNLINK_ACCOUNT { ...account, idempotency_key }`: This is enqueued when an account is to be unlinked.
- `ENQUEUE GET_USER { user_id }`: This is enqueued when a user is to be retrieved.
- `ENQUEUE GET_USER_BY_EMAIL { email }`: This is enqueued when a user is to be retrieved by email.
- `ENQUEUE GET_USER_BY_ACCOUNT { providerAccountId, provider }`: This is enqueued when a user is to be retrieved by account.
- `ENQUEUE GET_SESSION_AND_USER { session_id }`: This is enqueued when a session is to be retrieved.
...
**Danger: As noted above, by default, no caches have a TTL. This means that session tokens and verification tokens will never expire. This is useful for development, but should be changed in production.**

Caches are key-value stores in Redis. The following key prefixes are defined:
- `USER_DATA`: This is the key prefix for user data. The key is the user id.
- `EMAIL_INDEX`: This is the key prefix for email index data. The key is the email.
- `ACCOUNT_INDEX`: This is the key prefix for account index data. The key is a concatenation of the provider and providerAccountId, separated by a colon.
- `SESSION_DATA`: This is the key prefix for session data. The key is the session id.
- `VERIFICATION_REQUEST_DATA`: This is the key prefix for verification request data. The key is the verification request id.
- `ACCOUNT_DATA`: This is the key prefix for account data. The key is the account id.
...
### Idempotency
This package ensures that every exposed operation is idempotent, except get operations. Idempotency keys are generated by creating a string of `user_id:operation_name:stringified_data`. This string is then hashed using MurmurHash3 to allow for predictable length and character set. Keeping track of idempotency keys, deduplication, and atomicity are the responsibility of the backend.
...
type AdapterConfig = {
  ...
  redis: RedisClient;
  // Granular control over which caches are enabled. Defaults to true for all.
  caches: {
    user: boolean;
    email: boolean;
    account: boolean;
